  ...
  constructor(private tmdbService: TmdbService) {
    ...
    ngOnInit(): void {
        this.tmdbService.getTopMovies().subscribe((data: any) => {
            ...
            this.startCarousel();
            this.ratings = Array(8).fill(0).map((x, i) => ({
    ...
    randomFirstNameAndLastName() {
        const firstNames = ['John', 'Jane', 'Jack', 'Jill', 'James', 'Jenny', 'Jasper', 'Jasmine', 'Jude', 'Julia', 'Frank', 'Fiona', 'Felix', 'Felicity', 'Finn', 'Fiona', 'Freddie', 'Faye', 'Gus', 'Greta', 'George', 'Gloria', 'Gavin', 'Gemma', 'Hank', 'Holly', 'Harry', 'Hannah', 'Henry', 'Hazel', 'Hugo', 'Heidi', 'Ivan', 'Ivy', 'Isaac', 'Isla', 'Ian', 'Irene', 'Igor', 'Iris', 'Jake', 'Jade', 'Jasper', 'Jasmine', 'Jude', 'Julia'];
        const lastNames = ['Johnson', 'Jackson', 'Jenkins', 'Jennings', 'Jensen', 'Johansson', 'Abernathy', 'Baker', 'Carter', 'Dawson', 'Fletcher', 'Gibson', 'Henderson', 'Kendrick', 'Lawson', 'Morrison', 'Nicholson', 'Patterson', 'Richardson', 'Simpson', 'Thompson', 'Watson', 'Wilkinson', 'Williamson'];
